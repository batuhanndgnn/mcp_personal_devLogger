...
Proje Hakkında
Yazılım geliştirme süreçlerinde karşılaşılan spesifik hatalar, framework davranışları ve bulunan çözümler genellikle not defterlerinde veya Slack/Discord kanallarında dağınık halde kalır. Aynı hatayla aylar sonra tekrar karşılaşıldığında, çözümü hatırlamak ciddi bir zaman kaybı yaratır.

Bu proje, yerel bilgisayarınızda çalışan bir SQLite veritabanını Anthropic'in Model Context Protocol (MCP) standardı üzerinden doğrudan Claude Desktop'a bağlar. Bu sayede Claude, sohbet sırasında otonom olarak hata notlarınızı kaydedebilir, güncelleyebilir, silebilir ve geçmişte karşılaştığınız bir sorunu saniyeler içinde tarayarak size çözümünüzü sunabilir.
...
Tam Kontrol (CRUD): Sadece not eklemekle kalmaz; hatalı notları silebilir veya eski çözümlerinizi güncel bilgilerle revize edebilirsiniz.

Otonom Orkestrasyon: LangChain gibi ekstra çerçeveler kullanılmamıştır. İsteklerin analizi Claude'un kendi otonom döngüsüne bırakılmıştır.

Sunulan Araçlar (MCP Tools)
Claude'un kullanımına sunulan 5 temel araç bulunmaktadır:
...
Arama: "Geçen ay aldığım Postgres port problemini nasıl çözmüştüm, veritabanımdan kontrol eder misin?"
